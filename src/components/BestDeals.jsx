...
const BestDeals = ({ bestDeals }) => {
  ...
  useEffect(() => {
    getProducts({ bestDeal: true, limit: 5 })
      .then(res => {
        ...
        <div className="product-section__header">
          <div>
            <h2 className="product-section__title">
              {bestDeals?.sectionTitle || 'Best Deal Offer!'}
            </h2>
            <p className="product-section__sub">
              {bestDeals?.sectionSubText || 'Handpicked compliance solutions from verified industry experts.'}
            </p>
          </div>
